    ...
    <div className="absolute inset-0 overflow-hidden bg-ink text-paper">
      ...
      <div className="absolute inset-x-5 bottom-4 top-[4.5rem] flex flex-col md:top-[5.5rem]">
        ...
        <div className="border-b border-paper/15 pb-3 md:pb-4">
          <h2 className="w-[72%] font-black uppercase leading-[0.86] tracking-[-0.03em] md:w-[58%] lg:w-[44%]" style={display}>
        ...
        {/* Body */}
        <div className="mt-5 grid min-h-0 flex-1 content-center gap-8 md:mt-7 lg:content-stretch lg:grid-cols-[1.15fr_1fr] lg:gap-16">
          {/* Left: statement, bio, where */}
          <div className="flex min-w-0 flex-col justify-center">
            <p className="max-w-[32rem] text-[clamp(22px,3.4vw,40px)] font-medium leading-[1.06] tracking-[-0.015em] lg:text-[clamp(24px,2.6vw,40px)]">
          ...
          {/* Phones & tablets: links live here (the landing keeps its bottom clear for the photo) */}
          <ul className={`flex flex-wrap gap-x-5 gap-y-2 pt-1 lg:hidden ${label}`}>
            {[...site.links, { label: 'Email', href: `mailto:${site.email}` }].map((l, i) => (
              <li key={l.label}>
                <Line play={textIn} delay={0.55 + i * 0.05}>
                  <a
                    href={l.href}
